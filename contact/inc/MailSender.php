...
class MailSender {
    ...
    /**
     * When email sender is set try to set the envelope sender in sendmail application.
     * @var bool
     */
    private $disableEnvelopeSender;
    ...
    public function __construct($config) {
        ...
        $this->disableEnvelopeSender = in_array('disableEnvelopeSender', $config) ? !!$config['disableEnvelopeSender'] : false;
    ...
    public function sendMessage() {
        ...
        if (!$this->hasValidToken()) {
            ...
            $result = mail(
                ...
                $this->getHeaders(),
                !$this->disableEnvelopeSender && strlen($this->emailSender) > 0 ? '-f '.$this->emailSender : null
            );
